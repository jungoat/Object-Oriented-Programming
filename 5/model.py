...
class SampleReader:
    target_class = Sample
    header = ["sepal_length", "sepal_width", "petal_length", "petal_width", " calss"]
...
class KnownSample(Sample):
    ...
    def __init__(
        ...
        self.pupose = purpose_enum
        self.species = species
        self._classification: Optional[str] = None

    def matches(self) -> bool:
        return self.species == self.calssification
    ...
    def classification(self) -> Optional[str]:
        if self.purpose == Purpose.Testing:
            ...
            raise AttributeError(f"Training samples have no calssification")
